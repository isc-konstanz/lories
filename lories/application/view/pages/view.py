...
class View(PageGroup):
    ...
    def __init__(self, id: str, header: PageHeader, footer: PageFooter, *args, **kwargs) -> None:
        ...
        # Sub-pages of non-PageGroup ComponentPages (e.g. stations of a charge controller).
        # They must be tracked separately so they don't pollute their parent's card layout
        # while still getting their own Dash layout created and registered.
        self._sub_pages: List[Page] = []
    ...
    def _create_page(self, view: PageGroup, component: Component) -> Optional[Page]:
        ...
        if page is not None:
            ...
            if isinstance(page, PageGroup):
                ...
                # ComponentPage (non-PageGroup): sub-components still need their own Dash
                # pages so that the hrefs generated by _build_component_item resolve.
                # We cannot append them to `page` (not a PageGroup), so they are collected
                # in _sub_pages and handled in _at_create_layout / _at_register.
                for page_component in page.components.values():
                    self._create_component_sub_page(page, page_component)

            view.append(page)
        return page

    def _create_component_sub_page(self, parent: Page, component: Component) -> Optional[Page]:
        """Create a Dash page for *component* whose UI parent is a plain ComponentPage.

        Because ``parent`` is not a PageGroup we cannot call ``parent.append()``.
        Instead the page is appended to ``self._sub_pages`` so it can be layout-created
        and registered independently without appearing as a card in the parent's overview.
        The ``group=parent`` argument ensures the generated ``path`` matches the ``href``
        produced by :meth:`ComponentPage._build_component_item`.
        """
        if not component.is_enabled():
            self._logger.debug(f"Skipping page creation for disabled {type(component).__name__} '{component.id}'")
            return None
        ...
        if registry.has_page(_type):
            registration = registry.get_page(_type)
            page = registration.initialize(component=component, group=parent)
        else:
            from lories.application.view.pages.components.page import ComponentPage

            page = ComponentPage(component=component, group=parent)

        if page is not None:
            self._sub_pages.append(page)

            if isinstance(page, PageGroup):
                # Registered sub-page is itself a group: use the normal recursive path
                # so its own children are attached to page._pages correctly.
                for page_component in page.components.values():
                    self._create_page(page, page_component)
            else:
                # Keep recursing for deeper nesting.
                for page_component in page.components.values():
                    self._create_component_sub_page(page, page_component)
    ...
    def _at_register(self) -> None:
        ...
        for page in self._connector_pages:
            if page.is_created():
                page.register()
        # Register sub-pages of non-PageGroup components.
        # _register also recurses into any PageGroup that ended up in _sub_pages.
        _register(self._sub_pages)

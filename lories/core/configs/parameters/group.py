    ...
    *required* defaults to ``False`` for groups because an entire section
    ...
    def __init__(
        ...
        if children:
            for child in children:
                # Inline children have no name yet — use their key instead.
                if child.name is None:
                    child.name = child.key
                child_key = child._resolve_key()
    ...
    def resolve(self, configs: "_Configurations") -> Optional[Dict[str, Any]]:
        ...
        if not configs.has_member(k):
            ...
            # Optional and absent — children do not apply.
            return None

        member = configs.get_member(k)
        return self._resolve_children(member)

    def _resolve_children(self, configs: "_Configurations") -> Dict[str, Any]:
        result: Dict[str, Any] = {}
        for key, child in self.children.items():
            result[key] = child.resolve(configs)
        return result

...
# Alias for built-in ``type()`` so the parameter named ``type`` does not shadow it.
_builtins_type = type
...
class SelectParameter(_TypedParameter):
    ...
    def __init__(
        ...
        if type is None:
            type = _builtins_type(choices[0])  # noqa: A001
        ...
        self._accessor = _ACCESSOR.get(type, lambda cfg, k: cfg.get(k))  # type: ignore[arg-type]

    def _get_typed(self, configs: "_Configurations", key: str) -> Any:
        return self._accessor(configs, key)

    def to_schema(self) -> dict:
        type_name = _builtins_type(self.choices[0]).__name__ if self.choices else "str"

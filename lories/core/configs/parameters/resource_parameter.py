# -*- coding: utf-8 -*-
"""
lories.core.configs.parameters.resource_parameter
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Descriptor for declaring per-resource config keys on a ``Connector`` subclass.
"""
...
class ResourceParameter:
    """Declares a per-resource config key expected by a ``Connector`` subclass.

    These are the extra keys that live in the resource's connector-side config
    dict (e.g. ``address``, ``function``, ``device`` for Modbus; ``topic`` for
    MQTT).  ``ConnectorMeta`` collects them into ``__resource_parameters__`` and
    ``Connector._warn_undeclared_resource_configs()`` uses them at connect-time
    to warn about resource config keys that have no declaration.

    Intentionally **not** a subclass of ``_Parameter`` so that
    ``ConfiguratorMeta`` does not accidentally add it to
    ``__config_parameters__``.

    Only active at DEBUG log level — zero overhead in production.  Disable
    per-class by setting ``_WARN_UNDECLARED_resource_CONFIGS = False``.

    Usage::

        class ModbusClient(Connector):
            address  = resourceParameter(type=int, desc="Register start address")
            ...
            device   = resourceParameter(type=int, required=False, desc="Slave device ID")

        class MqttConnector(Connector):
            topic = resourceParameter(type=str, desc="MQTT topic for this resource")

    Parameters mirror :class:`~lories.core.configs.parameters.Parameter` but
    operate on the resource-level config dict rather than a ``Configurations``
    object.  Validation / resolution is intentionally left to the connector
    itself — ``resourceParameter`` is a *documentation / dev-aid* descriptor.
    """

    #: Attribute name on the owning class — injected by ``ConnectorMeta``.
    name: str | None
    #: Explicit override for the key in the resource config dict.
    key: str | None
    #: Human-readable description.
    desc: str | None
    #: Whether the resource config key is required.
    required: bool
    ...
    def _resolve_key(self) -> str:
        """Return the effective resource config key for this parameter."""

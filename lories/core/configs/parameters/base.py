...
class _TypedParameter(_Parameter, ABC):
    ...
    def _validate(self, key: str, value: Any) -> None:
        ...
        if self.choices is not None and value.lower() not in self.choices:
            raise _config_error(f"Invalid value {(value.lower())!r} for '{key}': " f"must be one of {self.choices}")

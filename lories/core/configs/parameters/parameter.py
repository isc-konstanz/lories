...
*type* argument, keeping call sites identical to the old single-class API.
``lories.util.parse_type`` is used so that string type names (e.g. ``"int"``,
``"pandas.Timestamp"``) are accepted in addition to plain Python types.
"""
...
# Map of resolved Python types → typed subclass.
_TYPE_MAP: dict = {
    str: StringParameter,
    int: IntParameter,
...
# Shared kwargs forwarded to every typed subclass constructor.
...
def Parameter(
    ...
    min: Optional[Any] = None,  # noqa: A002 — numeric range lower bound
    max: Optional[Any] = None,  # noqa: A002 — numeric range upper bound
    **kwargs,
) -> _TypedParameter:
    ...
    # Resolve string type names (e.g. "int", "pandas.Timestamp")
    ...
    # Unknown type → constructor-cast fallback via CustomParameter

...
class View(ComponentGroup):
    ...
    def _create_menu_item(self) -> DropdownMenu:
        ...
        return DropdownMenu(
            children=[
                DropdownMenuItem(label, header=True),
                *[DropdownMenuItem(p.name, href=p.path) for p in self]
            ],
            nav=True,
            in_navbar=True,
            label=label,
